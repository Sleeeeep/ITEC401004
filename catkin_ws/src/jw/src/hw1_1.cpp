...
void initGrid(Mat &display, int nImageSize)
{
    ...
    line(display, Point(0, imageCenterCooord[1]), Point(2*imageCenterCooord[0], imageCenterCooord[1]), Scalar(0, 0, 255), 1);
    line(display, Point(imageCenterCooord[0], 0), Point(imageCenterCooord[0], 2*imageCenterCooord[1]), Scalar(0, 255, 0), 1);
}
...
void drawTrajectory(Mat &display, vector<Vec3d> &trajectory, double dMaxDist)
{
    ...
    for(int i=0; i<nSize; i++) 
    {
        ...
        if(fabs(path[pathcount].x-trajectory[i][0]) < 0.1 && fabs(path[pathcount].y-trajectory[i][1]) < 0.1)
            pathcount++;
    }
}

void drawPath(Mat &display, double dMaxDist)
{
    Vec2i imageHalfSize = Vec2i(display.cols/2, display.rows/2);

    for(int i=0; i<pathcount; i++) 
    {
        int x = imageHalfSize[0] + (path[i].x/dMaxDist)*imageHalfSize[0];
        int y = imageHalfSize[1] + (path[i].y/dMaxDist)*imageHalfSize[1];
        
        char pathtemp[20];
        sprintf(pathtemp, "(%.2lf, %.2lf)", path[i].x, path[i].y);
        
        circle(display, Point(x, y), 2, CV_RGB(0, 255, 255), 1, CV_AA);
        putText(display, pathtemp, Point(x, y), FONT_HERSHEY_SIMPLEX, 0.4, CV_RGB(255, 255, 0));
    }
}

void drawCurrentPositionWithRotation(Mat &display, Vec3d &xyz, Vec3d &rpy, double dMaxDist)
{
    const int nHeadingSize = 10;
...
{
    if(ac != 2) {
        printf(">> rosrun jw hw1_1 filename\n");
        return 1;
    }
    ...
    for(int i=0; i<n; i++)
        fscanf(fp, "%lf %lf", &path[i].x, &path[i].y);
        
    // Initialize the ROS system
    ros::init(ac, av, "hw1_1");
    ros::NodeHandle nh;
